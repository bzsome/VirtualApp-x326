...
import androidTool.DeviceUtils;
...
public class HomeActivity extends VActivity implements HomeContract.HomeView {
    ...
    private void initMenu() {
        ...
        menu.add("关于").setIcon(R.drawable.ic_settings).setOnMenuItemClickListener(item -> {
            AlertDialog.Builder normalDialog =
                    new AlertDialog.Builder(this);
            normalDialog.setIcon(android.R.drawable.stat_notify_chat);
            normalDialog.setTitle("软件关于").setMessage("\t\t\t\t沙盒/多开软件，虚拟空间\n\n\t\t此软件基于asLody的VirtualApp，请尊重版权。此软件仅供学习交流，禁止在未经授权的范围内使用。\n\n\t编译汉化：陈光超(QQ:152255720)\n\t编译时间：2018.02.02 (326)\n\t软件版本：CTDU-1506032-专版");
            normalDialog.setPositiveButton("在线帮助",
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            Uri content_url = Uri.parse("http://www.bzchao.com");
                            intent.setData(content_url);
                            startActivity(intent);
                        }
                    });
            normalDialog.setNeutralButton("联系QQ",
                    new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            // ...To-do
                            try {
                                String url = "mqqwpa://im/chat?chat_type=wpa&uin=153355720";
                                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url)));
                            } catch (Exception e) {
                                Toast.makeText(getContext(), "QQ启动失败！可能您的手机暂未安装QQ客户端", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
    ...
    private void deleteApp(int position) {
        ...
        new AlertDialog.Builder(this)
                .setTitle("删除应用")
                .setMessage("您将要删除与应用： " + data.getName() + "?")
